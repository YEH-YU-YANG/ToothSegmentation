...
# def load_volume(volume_dir: Path):
#     """
#     Load the largest (by #files) DICOM series in this directory.
#     This avoids "randomly picking the first series" when multiple series exist.
#     """
#     reader = sitk.ImageSeriesReader()
#     series_ids = reader.GetGDCMSeriesIDs(str(volume_dir))
#     if not series_ids:
...
#         fns = reader.GetGDCMSeriesFileNames(str(volume_dir), sid)
#         n = len(fns)
#         if n > best_n:
#             best_n = n
#             best_id = sid
...
#     if not best_files:
#         raise ValueError(f'No files found for series in "{volume_dir}"')

#     reader.SetFileNames(best_files)
#     return reader.Execute()


# def reorient_volume(volume: sitk.Image, orient_code: str = "LPS") -> sitk.Image:
#     """
#     Force a consistent anatomical axis order/sign across all cases.
#     This fixes the "some cases mirrored / upside-down" issue caused by differing DICOM directions.
#     """
#     f = sitk.DICOMOrientImageFilter()
#     f.SetDesiredCoordinateOrientation(orient_code)
...
#     arr = sitk.GetArrayFromImage(volume)  # [z, y, x]
#     for idx, img in enumerate(arr, start=0):
#         # view='stl' wants neurological: image-left == patient-left
#         # After reorient to LPS, default displayed axial is radiological, so we flip L/R for stl.
#         if flip_lr:
#             img = img[:, ::-1]
...
#     # NEW: canonical orientation to eliminate random flips between cases
#     parser.add_argument(
#         "--orient",
#         type=str,
#         default="LPS",
#         help="Force volume to a canonical patient orientation before saving PNGs. Recommended: LPS.",
#     )
...
#         choices=["stl", "dicom"],
#         default="stl",
#         help="PNG display convention. 'stl' flips L/R AFTER canonical orientation (default).",
#     )
...
#             if cbct_only:
#                 out_rel = output_rel_for_cbct(series_dir, dicom_root) or src_rel
#             else:
#                 out_rel = src_rel
...
#                 # KEY FIX: normalize orientation first (solves random flips across cases)
#                 volume = reorient_volume(volume, args.orient)
...
# Example:
# python dcm2png.py --dicom "D:\YYYEH\TOOTHSEGMENTATION\data" --output "D:\YYYEH\TOOTHSEGMENTATION\data" --orient LPS --view stl --overwrite
# python dcm2png.py --dicom "D:\YYYEH\TOOTHSEGMENTATION\data\40657603" --output "D:\YYYEH\TOOTHSEGMENTATION\data\40657603" --orient LPS --view stl --overwrite
